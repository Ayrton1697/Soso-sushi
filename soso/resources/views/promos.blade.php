@extends('layouts.layout')

    @section('title', 'Soso Sushi|Promos')

    @section('content')

        <section class="section-promo--2">
        <div class="swiper--promo">
                <span class="swiper--promo--heading">10% OFF</span> 
                <span class="swiper--promo--sub">Take Away Abonando</span> 
                <span class="swiper--promo--sub">en efectivo</span> 
        </div>
        </section>

        <section class="section-black">

        </section>

        <section class="section-promo--1">
            <div class="swiper--promo">
                <span class="swiper--promo--sub">Promo exclusiva WHATSAPP</span> 
                <span class="swiper--promo--heading">Viernes y sábado</span> 
            </div>
            <div class="button--placer--promos">
                    <span 
                    class="btn btn--promo">Consultá la promo</span>
                  </div>
        </section>

        <section class="section-black">

        </section>
        
        <section class="section-promo--3">
            <div class="swiper--promo">
                <span class="swiper--promo--heading">Promo LUNES</span> 
                <span class="swiper--promo--sub">En combos premium</span> 
            </div>
            <div class="button--placer--promos">
                    <span 
                    class="btn btn--promo">20% OFF</span>
                  </div>
        </section>

        <section class="section-black">

        </section>
        
        <section class="section-promo--4">
            <div class="swiper--promo">
                <span class="swiper--promo--heading">Promo MARTES</span> 
                <span class="swiper--promo--sub">Sushi burger a eleccion</span> 
            </div>
            <div class="button--placer--promos">
                    <span 
                    class="btn btn--promo">2 X $885</span>
                  </div>
        </section>

        <section class="section-black">

        </section>
        
        <section class="section-promo--5">
            <div class="swiper--promo">
                <span class="swiper--promo--heading">Promo MIERCOLES</span> 
                <span class="swiper--promo--sub">X 15 piezas a elección sushiman</span> 
            </div>
            <div class="button--placer--promos">
                    <span
                    class="btn btn--promo">$490</span>
                  </div>
        </section>

        <section class="section-black">

        </section>

        <section class="section-promo--6">
            <div class="swiper--promo">
                <span class="swiper--promo--heading">Promo JUEVES</span> 
                <span class="swiper--promo--sub">Sushi salad</span> 
            </div>
            <div class="button--placer--promos">
                    <span 
                    class="btn btn--promo">2 X $825</span>
                  </div>
        </section>

        
    @endsection